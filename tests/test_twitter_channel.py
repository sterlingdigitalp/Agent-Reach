...
def test_saved_twitter_credentials_reach_probe_environment(tmp_path):
    config = Config(config_path=tmp_path / "config.yaml")
    config.set("twitter_auth_token", "saved-auth")
    config.set("twitter_ct0", "saved-ct0")
    probe = Mock(status="ok", ok=True, output="ok: true")

    with patch("agent_reach.channels.twitter.probe_command", return_value=probe) as call:
